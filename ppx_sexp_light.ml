...
let rec dot_qualifier ident =
  match ident with
  | Lident name -> name
  | Ldot (left, name) -> Printf.sprintf "%s.%s" (dot_qualifier left) name
  | Lapply _ -> failwith "unreachable"
;;

let sexp_of_type (typ : label_declaration) : string =
  let t = typ.pld_type.ptyp_desc in
  match t with
  | Ptyp_any -> "sexp_of_any"
  | Ptyp_var _ -> "sexp_of_var"
  | Ptyp_arrow (_, _, _) -> "sexp_of_arrow"
  | Ptyp_tuple _ -> "sexp_of_tuple"
  | Ptyp_constr ({ loc = _; txt = qualified_type }, []) ->
    (match qualified_type with
     | Lident name -> Printf.sprintf "sexp_of_%s" name
     | Ldot (modul, name) -> Printf.sprintf "%s.sexp_of_%s" (dot_qualifier modul) name
     | Lapply (_, _) -> "sexp_of_apply")
  | Ptyp_constr (_, _) -> "sexp_of_constr"
  | Ptyp_object (_, _) -> "sexp_of_object"
  | Ptyp_class (_, _) -> "sexp_of_class"
  | Ptyp_alias (_, _) -> "sexp_of_alias"
  | Ptyp_variant (_, _, _) -> "sexp_of_variant"
  | Ptyp_poly (_, _) -> "sexp_of_poly"
  | Ptyp_package _ -> "sexp_of_package"
  | Ptyp_open (_, _) -> "sexp_of_open"
  | Ptyp_extension _ -> "sexp_of_extension"
;;
...
let generate_sexp_of_field ~loc record_exp field_declaration =
  sexp_list
    ...
    ; pexp_apply
        ~loc
        (pexp_ident ~loc { loc; txt = lident (sexp_of_type field_declaration) })
